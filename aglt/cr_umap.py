...
def curvature_regularized_umap(
    ...
    derived from the graph's Ricci curvature. The regularization term encourages
    nodes with high positive average curvature (i.e., hubs within clusters)
    to be closer in the embedding space.
    ...
    graph : networkx.Graph
        The graph representation of the data, with 'ricciCurvature' as an
        edge attribute. It MUST have the same number of nodes as n_samples.
    n_components : int, optional
        The dimension of the space to embed into. Defaults to 2.
    n_epochs : int, optional
        The number of optimization epochs to run. Defaults to 200.
    learning_rate : float, optional
        The initial learning rate for the optimization. Defaults to 1.0.
    gamma : float, optional
        The weight of the curvature regularization term. Higher values
        increase the influence of the geometric prior. Defaults to 1.0.
    ...
    w_prime_matrix = coo_matrix(
        (reg_weights, (reg_rows, reg_cols)), shape=(n_samples, n_samples)
    ).tocsr()
