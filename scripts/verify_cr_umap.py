...
def main():
    ...
    for i in range(len(data)):
        for j in range(i + 1, len(data)):
            ...
            if (labels[i] == labels[j] and dist < 1.5) or \
               (labels[i] != labels[j] and labels[j] == 2 and dist < 3) or \
               (labels[j] != labels[i] and labels[i] == 2 and dist < 3):
                G.add_edge(i, j)
    ...
    # 3. Run CR-UMAP
    embedding = js.curvature_regularized_umap(
        data, orc.G, n_epochs=100, gamma=5.0
    )
